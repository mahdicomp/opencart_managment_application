...
class ControllerExtensionApi32AccountReward extends Controller {
	public function index() {
		...
		$data['rewards'] = array();
       $data['total']=$this->currency->format(0, $this->session->data['currency']);
		$filter_data = array(
			'sort'  => 'date_added',
			'order' => 'DESC',
			'start' => ($page - 1) * 10,
		...
		foreach ($results as $result) {
			$data['rewards'][] = array(
				...
				'points'      => $this->currency->format($result['points'], $this->session->data['currency']),
		...
		$data['total'] = $this->currency->format((int)$this->customer->getRewardPoints(), $this->session->data['currency']);
