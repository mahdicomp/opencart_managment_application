<?php
class ControllerExtensionApi32CheckoutConfirm extends Controller {
	public function index() {
	      if($this->valid($this->request->get['token'])){
		   ...
		   $this->load->controller('extension/api32/common/currency');
			...
			if(isset($this->request->post['pincodedays_deliverydate'])){
			$this->session->data['pincodedays_deliverydate']=$this->request->post['pincodedays_deliverydate'];
			$this->session->data['pincodedays_timeslot']=$this->request->post['pincodedays_timeslot'];
			}
			
		 	$this->session->data['payment_method']['code']=$this->request->post['payment_method'];
		$this->session->data['payment_title']['title']=$this->request->post['payment_title'];
		...
		foreach ($products as $product) {
			...
			$order_data['payment_method'] = $this->request->post['payment_title'];
			} else {
			...
			if($order_data['currency_id']==0){
               $order_data['currency_id']=2;
             }
             if(!$order_data['currency_code']){
                  $order_data['currency_code']=$this->config->get('config_currency');
             }
             if($order_data['currency_value']==0.00000000){
			...
			if($this->session->data['payment_method']['code']=='cod' || $this->session->data['payment_method']['code']=='bank_transfer' ){
			...
			$data['success'] = $this->load->controller('extension/payment/' . $this->session->data['payment_method']['code']);
