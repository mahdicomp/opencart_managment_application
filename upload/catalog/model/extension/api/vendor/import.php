 ...
 * 
 */
class ModelExtensionApiVendorImport extends Model {
		...
		public function category($category,$parent_id,$store_id,$language_id)
		{
			...
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" .$this->db->escape(str_replace("'",'',$category)). "' and store_id='".$store_id."' and language_id='".$language_id."'");
			if(!$query->row)
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape(str_replace("'",'',$category)) . "',store_id='".$store_id."',language_id='".$language_id."'");
			}
			else
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($category) .'-'.$category_id."',store_id='".$store_id."',language_id='".$language_id."'");
			}
			return $category_id;
			}
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" .$this->clean($barnd). "' and store_id='".$store_id."' and language_id='".(int)$this->config->get('config_language_id') ."'");
		if(!isset($query->row))
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'manufacturer_id=" . (int)$manufacturer_id . "', keyword = '" . $this->clean($barnd) . "',store_id='".$store_id."',language_id='".$language_id."'");
			}
			else
			{
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'manufacturer_id=" . (int)$manufacturer_id . "', keyword = '" . $this->clean($barnd) .'-'.$manufacturer_id."',store_id='".$store_id."',language_id='".$language_id."'");
			}
	...
	{
		$query = $this->db->query("SELECT p.product_id FROM `" . DB_PREFIX . "product` p left join `" . DB_PREFIX . "vendor_to_product` vt on vt.product_id=p.product_id  where p.model='".$model."' AND vt.vendor_id = '" . $vendor_id . "'");
	...
	$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "'");
		...
		if (isset($data['product_category'])) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'product_id=" . (int)$product_id. "' and language_id='".$language_id."'");
		
		if ($data['keyword']) {
			$data['keyword']=$this->clean($data['keyword']);
			if (isset($data['product_store'])) {
			foreach ($data['product_store'] as $store_id) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'product_id=" . (int) $product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "',store_id='".$store_id."',language_id='".$language_id."'");
			}
			}
		}
