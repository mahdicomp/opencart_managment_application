<?php
class ModelExtensionApiVendorProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		$querystatus=$this->db->query("select status from " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");		
		if($autoapprovedproduct==1){
			$vstatus =  (int)$data['status'];
		} else if($autoapprovedproduct==0) {
			if($querystatus->row['status']==2) {
				$vstatus = '2';
			} 
