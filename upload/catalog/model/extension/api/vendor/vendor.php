<?php
class ModelExtensionApiVendorVendor extends Model {
	public function addVendor($data) {
		
		$autoapprovedseller =  $this->config->get('vendor_vendorautoapprove');
		
		$sql = "INSERT INTO " . DB_PREFIX . "vendor SET display_name = '" . $this->db->escape($data['display_name']) . "',firstname = '" . $this->db->escape($data['firstname']) . "',lastname = '" . $this->db->escape($data['lastname']) . "',email = '" . $this->db->escape($data['email']) . "',telephone = '" . $this->db->escape($data['telephone']) . "',fax = '" . $this->db->escape($data['fax']) . "',company = '" . $this->db->escape($data['company']) . "',address_1 = '" . $this->db->escape($data['address_1']) . "',address_2 = '" . $this->db->escape($data['address_2']) . "',map_url = '" . $this->db->escape($data['map_url']) . "',city = '" . $this->db->escape($data['city']) . "',country_id = '" .$data['country_id'] . "',zone_id = '" .$data['zone_id'] . "',salt = '" . $this->db->escape($salt = token(9)) . "',password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "',status = '1',approved = '".$autoapprovedseller."',about = '" . $this->db->escape($data['about']) . "',image='".$data['image']."',logo='".$data['logo']."',store_about='".$this->db->escape($data['store_about'])."',banner='".$data['banner']."',payment_method='".$this->db->escape($data['payment_method'])."',paypal = '" . $this->db->escape($data['paypal']) . "', bank_name = '" . $this->db->escape($data['bank_name']) . "', bank_branch_number = '" . $this->db->escape($data['bank_branch_number']) . "', bank_swift_code = '" . $this->db->escape($data['bank_swift_code']) . "', bank_account_name = '" . $this->db->escape($data['bank_account_name']) . "', bank_account_number = '" . $this->db->escape($data['bank_account_number']) . "',tax_number='".$this->db->escape($data['tax_number'])."',shipping_charge='".$this->db->escape($data['shipping_charge'])."',date_added = NOW(), postcode = '" .$this->db->escape($data['postcode']). "'";
		$this->db->query($sql);
	...
	public function getVendor($vendor_id=0){
		$sql = "SELECT *,v.vendor_id as vendor_id FROM " . DB_PREFIX . "vendor v LEFT JOIN " . DB_PREFIX . "vendor_description vd ON (v.vendor_id = vd.vendor_id) LEFT JOIN ".DB_PREFIX."vendor_review vr ON(v.vendor_id = vr.vendor_id) WHERE v.vendor_id='".$vendor_id."' AND v.approved!=0 AND vd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$query=$this->db->query($sql);
	...
	public function getTotalReviews($vendor_id){
		$sql="select count(*) as total from " . DB_PREFIX . "vendor_review where vendor_id='".$this->vendor->getId()."' AND status=1";
